...
TimeSpan.prototype = {
	...
	toString : function(includeSeconds, includeMilliseconds){
		return (this._millis < 0 ? "-" : "") + (Math.abs(this.days()) ? TimeSpan.pad(Math.abs(this.days()))  + ".": "") + TimeSpan.pad(Math.abs(this.hours())) + ":" + TimeSpan.pad(Math.abs(this.minutes())) + (includeSeconds ? ":" + TimeSpan.pad(Math.abs(this.seconds())) : "") + (includeMilliseconds ? "." + Math.abs(this.milliseconds()) : "");
	}
};
